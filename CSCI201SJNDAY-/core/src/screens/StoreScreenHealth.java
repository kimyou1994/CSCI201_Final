...
public class StoreScreenHealth extends StoreScreen implements Screen, InputProcessor {
	private SpriteBatch batch;
	private Nina nina;
	private String ninaPhrase;
	
	public StoreScreenHealth(ShootsAndLooters shootsAndLooters) {
	...
	public void show() {
		...
		for(int i = 0; i < 10; i++) {
			if(i <= game.getUsrData().getArmorLevel()) {
				healthUpgrades.get(i).setStyle(healthBStyle);
			}
		}
		...
		for(int i = 0; i < 3; i++) {
			...
			if(!(index >= 10)) {
				mainTable.add(healthUpgrades.get(index));
			} else {
				mainTable.add();
			}
		}
		...
		mainTable.add();
		mainTable.add();
		mainTable.add(BuyHealthButton).width(300f);
		mainTable.add();
		mainTable.add();
